...
public class ApiController {
    ...
    public List<Location> getLocations(@RequestParam(value = "city", defaultValue = "") String city,
        ...
        if (!city.isEmpty() && !parentCategory.isEmpty() && !category.isEmpty()) {
            return locationInterface.findByCityAndParentCategoryAndCategory(city, parentCategory, category);
        } else if (!city.isEmpty() && !parentCategory.isEmpty()) {
            return locationInterface.findByCityAndParentCategory(city, parentCategory);
        } else if (!parentCategory.isEmpty() && !category.isEmpty()) {
            return locationInterface.findByParentCategoryAndCategory(parentCategory, category);
        } else if (!city.isEmpty() && !category.isEmpty()) {
            return locationInterface.findByCityAndCategory(city, category);
        } else if (!city.isEmpty()) {
            return locationInterface.findByCity(city);
        } else if (!parentCategory.isEmpty()) {
            return locationInterface.findByParentCategory(parentCategory);
        } else if (!category.isEmpty()) {
            return locationInterface.findByCategory(category);
        }
